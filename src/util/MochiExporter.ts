...
class MochiExporter {
  ...
  async readCards(): Promise<Card[]> {
    ...
    let cards: Card[] = [];
    let accumulatingTerm = false;
    let card: Card = { term: "", content: "" };
  
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].contains(cardTag)) {
        if (accumulatingTerm) {
          // Finish current card and start a new one
          cards.push(card);
          card = { term: "", content: "" };
        }
        accumulatingTerm = true;
        // Start accumulating term
        card.term = lines[i].replace(cardTag, "").trim();
      } else if (lines[i].trim() === "#card") {
        // Separator encountered, start accumulating content
        accumulatingTerm = false;
      } else {
        if (accumulatingTerm) {
          // Accumulate term
          card.term += "\n" + lines[i];
